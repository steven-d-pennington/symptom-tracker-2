  ...
  const filteredFoods = useMemo(() => {
    ...
    return foods.filter(
      (f) =>
        ...
        f.allergenTags.some((tag) => tag.toLowerCase().includes(term))
    )
  }, [foods, searchTerm])
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-gray-600 dark:text-gray-400 mb-8 text-center">
        Choose foods you commonly eat ({totalSelected} selected)
      </p>

      <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="Search foods..." />

      {/* Custom Items Display */}
      {customItems.length > 0 && (
        <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
          <h3 className="font-semibold text-green-800 dark:text-green-200 mb-2">
            Custom Foods Added ({customItems.length})
          </h3>
      ...
      <div className="max-h-96 overflow-y-auto mb-6">
        {Object.entries(foodsByCategory).map(([category, categoryFoods]) => (
          <CategorySection key={category} category={category} itemCount={categoryFoods.length}>
            <div className="space-y-2">
                ...
                <SelectionCard
                  ...
                  description={
                    food.allergenTags.length > 0
                      ? `Allergens: ${food.allergenTags.join(', ')}`
                      : undefined
                  }
                  selected={selectedIds.includes(food.guid)}
                  onToggle={onToggle}
                />
              ))}
            </div>
      ...
      {!showCustomForm ? (
        ...
        <div className="mb-6 p-4 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">Add Custom Food</h3>
          <input
            type="text"
            value={customName}
